...
def obtener_datos_bvc(simbolo):
    """
    Obtiene el libro de órdenes desde la BVC usando el mismo endpoint que
    funciona en test_api.py (admin-ajax.php → getSimbolosDetalle).
    Devuelve un dict con claves 'bids' y 'asks', cada una lista de
    {'price': str, 'volume': str}, o None si falla.
    """
    try:
        session = requests.Session()
        session.get("https://www.bolsadecaracas.com/",
        ...
        params = {'action': 'getSimbolosDetalle'}
        data   = {'simbolo': simbolo.upper(), 'tipo': 'rv'}
        resp   = session.post(
        ...
        if not raw.get('success') or 'response' not in raw:
            return None

        response_data = raw['response']
        orderbook = None
        for key in ('cur_con_lib_ord_rv', 'cur_con_lib_ord_acc'):
            if (key in response_data and response_data[key]
                    and isinstance(response_data[key], list)
                    and len(response_data[key]) > 0):
                orderbook = response_data[key][0]
                break
        ...
        bids = []
        for i in range(1, 7):
            p = orderbook.get(f'PRE_CMP_{i}')
            v = orderbook.get(f'VOL_CMP_{i}', '0')
            if p not in (None, '', 'null'):
                bids.append({'price': str(p), 'volume': str(v)})

        asks = []
        for i in range(1, 7):
            p = orderbook.get(f'PRE_VTA_{i}')
            v = orderbook.get(f'VOL_VTA_{i}', '0')
            if p not in (None, '', 'null'):
                asks.append({'price': str(p), 'volume': str(v)})

        return {'bids': bids, 'asks': asks}

    except Exception as e:
        print(f"⚠️ obtener_datos_bvc({simbolo}): {e}")
        return None
...
def _extraer_lista_ordenes(datos_bvc):
    """
    Convierte la respuesta de obtener_datos_bvc() {'bids':[...], 'asks':[...]}
    en filas que espera el frontend y la cache:
    [{'PRE_CMP': x, 'VOL_CMP': x, 'PRE_VTA': x, 'VOL_VTA': x}, ...]
    """
    if not datos_bvc or not isinstance(datos_bvc, dict):
        return []
    bids = datos_bvc.get('bids', [])
    asks = datos_bvc.get('asks', [])
    ...
    for i in range(n):
        ...
        filas.append({
            'PRE_CMP': bid.get('price', ''),
            'VOL_CMP': bid.get('volume', ''),
            'PRE_VTA': ask.get('price', ''),
            'VOL_VTA': ask.get('volume', ''),
        })
    return filas

def _guardar_cache_libro(simbolo, lista_ordenes):
    """Persiste el libro de órdenes en la BD."""
    try:
        conn = get_db()
        ahora = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        conn.execute('''
            INSERT INTO libro_ordenes_cache (simbolo, datos_json, actualizado_en)
            VALUES (?, ?, ?)
            ON CONFLICT(simbolo) DO UPDATE SET
                datos_json     = excluded.datos_json,
                actualizado_en = excluded.actualizado_en
        ''', (simbolo, _json.dumps(lista_ordenes, ensure_ascii=False), ahora))
        conn.commit()
        conn.close()
    except Exception as e:
        print(f"⚠️ Error guardando caché libro {simbolo}: {e}")

def _leer_cache_libro(simbolo):
    """Lee el libro de órdenes guardado en BD. Devuelve (lista, timestamp) o (None, None)."""
    try:
        conn = get_db()
        row = conn.execute(
            "SELECT datos_json, actualizado_en FROM libro_ordenes_cache WHERE simbolo = ?",
            (simbolo,)
        ).fetchone()
        conn.close()
        if row:
            return _json.loads(row['datos_json']), row['actualizado_en']
    except Exception as e:
        print(f"⚠️ Error leyendo caché libro {simbolo}: {e}")
    return None, None
...
def api_libro_ordenes(simbolo):
    """
    Proxy al libro de órdenes de la BVC.
    - Si la BVC responde → devuelve datos en vivo Y los guarda en caché.
    - Si la BVC falla   → devuelve el último caché guardado con su timestamp.
    """
    simbolo = simbolo.upper().strip()
    datos_bvc = obtener_datos_bvc(simbolo)

    if datos_bvc is not None:
        lista = _extraer_lista_ordenes(datos_bvc)
        if lista:
            _guardar_cache_libro(simbolo, lista)
            return jsonify({"success": True, "datos": lista, "fuente": "en_vivo"})

    # BVC no disponible → intentar caché
    lista_cache, ts_cache = _leer_cache_libro(simbolo)
    if lista_cache:
        return jsonify({
            "success": True,
            "datos": lista_cache,
            "fuente": "cache",
            "cache_timestamp": ts_cache
        })

    return jsonify({"success": False, "datos": [],
                    "error": "BVC no disponible y no hay caché guardado para este símbolo"}), 200

# ── Endpoint admin: refrescar caché de todos los símbolos ──────────────────
@app.route('/admin/refrescar-libro-cache', methods=['POST'])
@login_required
def admin_refrescar_libro_cache():
    """
    Intenta descargar y cachear el libro de órdenes de todos los símbolos
    presentes en la BD. Útil para ejecutar cuando hay conexión disponible.
    """
    conn = get_db()
    simbolos = [r[0] for r in conn.execute(
        "SELECT DISTINCT simbolo FROM mercado ORDER BY simbolo"
    ).fetchall()]
    conn.close()

    ok, fallo = [], []
    for simb in simbolos:
        datos = obtener_datos_bvc(simb)
        if datos is not None:
            lista = _extraer_lista_ordenes(datos)
            if lista:
                _guardar_cache_libro(simb, lista)
                ok.append(simb)
                continue
        fallo.append(simb)
...
# ── Endpoint público: lista de símbolos en la BD ───────────────────────────
@app.route('/api/simbolos-mercado')
@api_permiso_requerido('ver_consulta')
def api_simbolos_mercado():
    """Devuelve la lista de símbolos únicos presentes en la tabla mercado."""
    conn = get_db()
    rows = conn.execute(
        "SELECT DISTINCT simbolo FROM mercado ORDER BY simbolo"
    ).fetchall()
    conn.close()
    return jsonify({"simbolos": [r['simbolo'] for r in rows]})
